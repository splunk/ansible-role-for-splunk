...
- **configure_shc_captain.yml** - Perform a `bootstrap shcluster-captain` using the server list built by `configure_shc_members.yml`. To override the server list, set `splunk_shc_uri_list` in your `group_vars`.
...
- **configure_shc_members.yml** - Initializes search head clustering on Splunk hosts that will be participating in a new search head cluster. Also builds `splunk_shc_uri_list` from `splunk_mgmt_uri` and `splunkd_port` across all hosts in `splunk_shc_target_group` (override by setting `splunk_shc_uri_list` in `group_vars`). Relies on the values of: `splunk_shc_key`, `splunk_shc_label`, `splunk_shc_deployer`, `splunk_shc_rf`, `splunk_shc_rep_port`, `splunk_mgmt_uri`, `splunkd_port`, `splunk_admin_username`, and `splunk_admin_password`. Be sure to review the default values for the role for these and configure them appropriately in your group_vars.
