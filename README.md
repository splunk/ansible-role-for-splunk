...
In order to use the app management functionality, apps are configured via the `apps` list. Each app entry specifies a `name` and a `download_app_source` of either `git` or `s3`, and both sources may be mixed within a single `apps` list and deployed in one run. The legacy `git_apps` variable is still honored for backward compatibility, so existing inventories continue to work unchanged. For git apps, configure `git_server`, `git_key`, `git_project`, and `git_version` (any of which may also be set per app down to the `name` level), and optionally `app_relative_path` to deploy a sub-directory of the repository or, with a single trailing slash, every app at the repository root. For S3 apps, configure `splunk_app_s3_bucket` and `splunk_app_s3_bucket_prefix` along with `s3_access_key_id` and `s3_secret_access_key` (omit both to use the controller's IAM role or credential chain); objects follow the `<prefix>/<app_id>/<version>/<tarball>` convention, where `version` is set per app, or a full `s3_object` key may be set on the app entry to override the convention and fetch an exact object. The S3 path requires the `amazon.aws` collection plus `boto3`/`botocore` on the controller. You will find additional examples in the included sample [group_vars](https://github.com/splunk/ansible-role-for-splunk/blob/master/environments/production/group_vars/deploymentserver.yml) and [host_vars](https://github.com/splunk/ansible-role-for-splunk/blob/master/environments/production/host_vars/my-shc-deployer.yml) files. You may also override the auto-configured `splunk_app_deploy_path` per app (for example, to deploy to $SPLUNK_HOME/etc/apps on a deployment server rather than the default $SPLUNK_HOME/etc/deployment-apps). If not set, configure_apps.yml determines the deploy path from the host's group membership.
