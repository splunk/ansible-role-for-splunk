...
In order to use the app management functionality, define an `apps` list where each entry names an app and a `download_app_source` of `git` or `s3` (both sources may be mixed in one run; the legacy `git_apps` variable is still honored). Apps deploy to `splunk_app_deploy_path`, auto-selected from the host's group membership and overridable globally or per app, and `splunk_apps_to_remove` removes apps explicitly. The schema, source-specific variables, validation rules, migration notes, and removal semantics are documented in [roles/splunk/tasks/splunk_apps/README.md](https://github.com/splunk/ansible-role-for-splunk/blob/master/roles/splunk/tasks/splunk_apps/README.md); sample configurations live in the included [group_vars](https://github.com/splunk/ansible-role-for-splunk/blob/master/environments/production/group_vars/deploymentserver.yml) and [host_vars](https://github.com/splunk/ansible-role-for-splunk/blob/master/environments/production/host_vars/my-shc-deployer.yml) files.
...
- **splunk_app_install.yml** - Install or upgrade apps on Splunk hosts using the configure_apps.yml task in the splunk role. Note that the apps you want to deploy should be defined in either host_vars or group_vars; splunk_app_deploy_path is selected automatically from group membership and may be overridden globally or per app. Refer to the documentation for app deployment for details.
...
- **configure_apps.yml** - This task should be called directly from a playbook in order to deploy apps or configurations (from git repositories or S3) to Splunk hosts, and to remove apps via `splunk_apps_to_remove`. It resolves and validates the app plan, then includes the internal task files under `tasks/splunk_apps/`. Tip: Add this task to a playbook after the check_splunk.yml play. Doing so will perform a "install (or upgrade) and deploy apps" run, all in one playbook.
...
- **splunk_apps/** - Internal task files implementing app deployment and removal (resolve, stage, install, remove); included by configure_apps.yml and not intended to be used directly as a `deployment_task`. See [roles/splunk/tasks/splunk_apps/README.md](https://github.com/splunk/ansible-role-for-splunk/blob/master/roles/splunk/tasks/splunk_apps/README.md) for the flow. You can configure the `rsync_private_key` variable to use a ssh key for rsync.
